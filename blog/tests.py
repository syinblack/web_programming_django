...
class TestView(TestCase):
    ...
    def test_create_post(self):
        ...
        # staff 가 아닌 일반 사용자(trump)가 로그인을 한다.
        self.client.login(username="trump", password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff 인 obama 가 로그인을 한다.
    ...
    def test_comment_update(self):
        ...
        comment_area = soup.find('div', id='comment-area')
        self.assertFalse(comment_area.find('a', id='comment-1-update-btn')) # 수정 버튼이 보이면 안됨.
        ...
        comment_area = soup.find('div', id='comment-area')
        self.assertFalse(comment_area.find('a', id='comment-2-update-btn'))         # 다른 유저의 댓글 수정 불가

        comment_001_update_btn = comment_area.find('a', id='comment-1-update-btn')  # 자신이 작성한 댓글만 수정 가능
    ...
    def test_delete_comment(self):
        ...
        comment_area = soup.find('div', id='comment-area')
        self.assertFalse(comment_area.find('a', id='comment-1-delete-btn'))     # trump가 작성한 두 번째 댓글에 대해서만 삭제 버튼 표시
        comment_002_delete_modal_btn = comment_area.find('a', id='comment-2-delete-modal-btn')  # 해당 버튼은 모달 호출
        ...
        delete_comment_modal_002 = soup.find('div', id='deleteCommentModal-2')  # delete 버튼 누른 모달화면
        ...
        response = self.client.get('/blog/delete_comment/2/', follow=True)  # delete 버튼을 클릭한 뒤 리다이렉트된 페이지
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
